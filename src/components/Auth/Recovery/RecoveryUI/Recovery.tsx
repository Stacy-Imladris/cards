...
export const Recovery = memo(({setEmail, email, toSendInstructions, isLoading, error, check, theme
                              }: RecoveryPropsType) => {
    return check
        ...
        : <div className={`${s.container} ${t[theme + '-text']}`}>
            ...
            <div><Link to="/login" className={`${s.link} ${t[theme + '-text']}`}>Try logging in</Link></div>
        </div>
})